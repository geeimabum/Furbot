...
def interact(input, rules, default_responses, user):
    ...
    return respond(rules, input, default_responses, user)


def respond(rules, input, default_responses, user):
    ...
    # Check for unique phrases for custom functions.
    response = custom.response(response, user)
    print response
    if response == 'blank':
        print "shouldn't reply"
        return False
    else:
        print response
        return response
